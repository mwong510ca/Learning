...
public class SeamCarver1Ddouble {
     ...
     */
    public SeamCarver1Ddouble(Picture picture) {
        ...
        for (int row = 1; row < height - 1; row++) {
            for (int col = 1; col < width - 1; col++) {
                picEnergy[idx] = calculateEnergy(idx++);
            }
            idx += 2;
        }
    }
    ...
    private void transpose() {
        ...
        double[] energyTrans = new double[size];
    ...
    public double energy(int x, int y) {
        ...
        return picEnergy[xyTo1d(y, x)];
    }
    
    // convert row, column to actural position in RGB pixel or energy storage
    ...
    private int[] getVerticalSeam() {
        ...
        if (width < 3 || height < 3) {
            ...
            double val = picEnergy[picIdx++];
            for (int col = 2; col < width - 1; col++) {
                double temp = picEnergy[picIdx++]; 
                if (temp < val) {
                    val = temp;
                    seam[1] = col;
                }
            ...
            int lastCol = width - 1;
            double[] weight = new double[(height - 3) * (width - 1) + 1];
            double max = BORDER;
            int idx = 0;
            weight[idx++] = max;
            ...
            seam[row] = 1;
            int picIdx = xyTo1d(row, 1);
            double val = picEnergy[picIdx++] + Math.min(weight[wtIdx++], weight[wtIdx]); 
            seam[row] = 1;
            for (int col = 2; col < lastCol; col++) {
                double temp = picEnergy[picIdx++] + Math.min(weight[wtIdx - 1],
                        Math.min(weight[wtIdx++], weight[wtIdx]));
                if (temp < val) {
                    val = temp;
                    seam[row] = col;
    ...
    private void eliminateVerticalSeam(int[] seam) {
        ...
        for (int row = 1; row < height - 1; row++) {
            int col = seam[row];
            if (col > 1) {
                picEnergy[row*picWidth + col-1] = calculateEnergy(xyTo1d(row, col - 1));
            }
            if (col < width - 1) {
                picEnergy[row* picWidth + col] = calculateEnergy(xyTo1d(row, col));
